...
nonReservedKeyword
    ...
    | PAGE | PAIMON | PARALLEL | PARQUET | PART | PARTIAL | PARTITION | PARTITIONS | PARTS | PASTE | PATCHES
    | PLAINTEXT_PASSWORD
    ...
    | RETURNS | REVOKE | REWRITE | ROLE | ROLES | ROLLBACK | ROW | ROWS | RUN | RUNNER
...
naturalJoinClause
    : GLOBAL? NATURAL (LEFT OUTER? | RIGHT OUTER? | FULL OUTER? | INNER)? JOIN tableExpressionAtom joinConstraint?
    ;
...
alterConstraint
    ...
    | MODIFY CONSTRAINT ifExists? identifier (CHECK | ASSUME) expr
...
describeStatement
    : (DESCRIBE | DESC) TEMPORARY? TABLE? TEMPORARY?
        (databaseAndTableName | tableFunctionCall | LPAREN selectUnion RPAREN (AS? identifier)? | selectUnion)
        FINAL?
        (settingsClause | formatClause)*
    ;
